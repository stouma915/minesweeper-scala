...
class SyncShowFlagPlaceholderIcon[F[_]: Sync: UpdateFlagPlaceholderIconVisibility]
    extends ShowFlagPlaceholderIcon[F] {

  override def perform(coord: Coordinate)(implicit document: HTMLDocument): F[Unit] =
    UpdateFlagPlaceholderIconVisibility[F].update(coord, true)
